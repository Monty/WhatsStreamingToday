...
function appendToFile(tabData, showURL, filePath, content) {
  fs.appendFile(filePath, content, (err) => {
    if (err) {
      console.error(`==> Error writing to file ${filePath}:`, err);
    } else {
      console.log(`==> Completed ${tabData} from ${showURL}`);
    }
  });
}

async function handleTabWithCombobox(page, tabName, writeDataFunction) {
  if (await elementExists(page, 'tab', tabName)) {
    ...
    if (await combobox.isVisible()) {
      ...
      for (const option of options) {
        ...
        await page.waitForTimeout(2500); // Adjust time as needed

        const tabContent = await page
          .getByRole('tabpanel', { name: tabName })
          .ariaSnapshot();
        await writeDataFunction(
          page,
      ...
      const tabContent = await page
        .getByRole('tabpanel', { name: tabName })
        .ariaSnapshot();
      await writeDataFunction(page, `${tabName} tab`, tabContent);
    }
  }
}
...
async function writeEpisodeData(page, title, snapshot) {
  const episodes = [];
  ...
  appendToFile(
    ...
    series_URL,
    output_file,
    `<!-- ${title} data from ${series_URL} -->\n${episodes.join('\n')}\n\n`
  );
}

async function writeEssentialData(title, source, eofString, offset) {
  let essentialData = '';
...
(async () => {
  ...
  try {
    ...
    await handleTabWithCombobox(page, 'Clips & Previews', writeEpisodeData);

    // Get episodes from the Special tab
    await handleTabWithCombobox(page, 'Special', writeEpisodeData);

    // Get episodes from the Episodes tab
    await handleTabWithCombobox(page, 'Episodes', writeEpisodeData);
  } catch (error) {
