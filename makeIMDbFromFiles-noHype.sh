#!/usr/bin/env bash
#
# Generate show and cast spreadsheets (WITHOUT HYPERLINKS) from:
...
KNOWNFOR_TITLES="IMDb_knownTitles-noHype$DATE_ID.txt"
...
PUBLISHED_KNOWNFOR_TITLES="$BASELINE/knownTitles-noHype.txt"
#
...
perl -p -e 's+^.*{++; s+}.*++;' $TCONST_KNOWN_PL | sort -fu >$KNOWNFOR_TITLES
...
# Sort by Person (1), Primary Title (2), Rank (4), Original Title (3)
sort -f --field-separator="$TAB" --key=1,2 --key=4,4 --key=3,3 \
