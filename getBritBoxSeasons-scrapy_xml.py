  ...
  # Works for /us/show/ with > 1 seson
    ...
    def parse(self, response):
        for page in self.start_urls:
            if "/episode/" in page:
                ...
                seasons = response.css('div.program-item__title-wrapper p.program-item__program-subtitle::text').extract_first()
    ...
    def parse1Episode(self, response):
        ...
        for i, item in enumerate(URLs):
            if (item == shortURL):
                URL = URLs[i]
                Title = response.css('div.program-item h3.program-item__program-title::text')[i].extract()
                Subtitle = response.css('div.program-item p.program-item__program-subtitle::text')[i].extract()
                Duration = response.css('div.program-item span.programme-metadata__duration::text')[i].re(r'(\d+)')
                Year = response.css('div.program-item span.programme-metadata__year::text')[i].extract()
                Rating = response.css('div.program-item span.programme-metadata__classification::text')[i].extract()
                yield {
                        'URL': URL,
                        'Title': Title,
                        'Subtitle': Subtitle,
